...
class ListsSuite extends munit.FunSuite:
  ...
  test("sum of one elem list") {
    assertEquals(sum(List(1)), 1)
  }

  test("sum with negative elem list") {
    assertEquals(sum(List(-1, 2)), 1)
  }
  ...
  test("max of one elem list") {
    assertEquals(max(List(2)), 2)
  }

  test("max with negative elem list") {
    assertEquals(max(List(-4, -2, 1)), 1)
  }
